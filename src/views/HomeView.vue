...
export default {
  ...
  data() {
    return {
      ...
      userData: null,
      userArtistsData: null,
     // isPlaying: false,
    }
  },
  beforeMount() {
    setInterval(() => this.authStore.init(), 3600000);
    console.log("TESTE" + this.authStore.init());
  },
  created() { // get access token | get user info
    try {
  ...
  methods: {
    async loadData() {
      try {
        ...
        this.userArtistsData = this.authStore.spotifyData
      } catch (error) {
        alert(error.message);
      }
    },
  ...
  computed: {
    getUserFollowers() {
      return this.authStore.getUserFollowers
    }
  },
}
</script>
  ...
  <main
    ...
    <div
      ...
      <div class="col-span-1 . md:col-span-1 | row-span-2 | bg-c_primary p-c_pad rounded-c_br shadow-c_shadow">
        <c_ImageText v-if="userData" :image="userData.images[0]?.url"
          :info="userData.display_name">
        </c_ImageText>
      </div>
      <div class="col-span-1 . md:col-span-1 | row-span-2 | bg-c_primary p-c_pad rounded-c_br shadow-c_shadow">
        <c_ImageText v-if="userData" :info="userData.product" image='/public/images/logo.png'></c_ImageText>
      </div>
      <div class="col-span-2 . md:col-span-1 | row-span-2 | bg-c_primary p-c_pad rounded-c_br shadow-c_shadow">
        <c_ImageText v-if="userData" info="Followers" :text="getUserFollowers"></c_ImageText>
      </div>
      <!--  -->
      <div
